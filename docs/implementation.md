...
Terraform is split into `network`, `security`, `ec2`, `alb`, `acm`, `verified-access`, and `monitoring` modules. The composition is under `terraform/environments/main` and uses one isolated state key. Optional state/OIDC bootstrap is separate so the state bucket is never created by the configuration that depends on it.
...
The optional bootstrap creates one repository-scoped plan role and one `main` deployment role. The plan role is read-only except for state lock objects, so pull requests can refresh and plan without infrastructure mutation authority. The deployment role trusts only the `main` GitHub Environment subject and is not AdministratorAccess. Its deployment service actions are broad because many EC2/Verified Access APIs do not support useful resource scoping; IAM role/profile operations are restricted to the matching project and environment prefix.
